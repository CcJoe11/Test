...
#define PI 3.1415925
#define angle_range 2*PI/3 //hit's range
...
ArmorFinder::ArmorFinder(const uint8_t &anti_top):is_anti_top(anti_top){
    anti_switch_cnt = 0;
    contour_area = 0;
    tracking_cnt = 0;
    _x = 0, _z = 0;
...
void ArmorFinder::set_target_box(ArmorDetector P)
{
    target_box = P;
}
void ArmorFinder::set_x_z(double angle)
{
    double r = 50;                     //need to be changed
    double belta = angle - angle_range/2;
    _x = r*sin(belta);
    int z0 = target_box.getIntDistance();
    _z = (z0 + r * (1-cos(belta))) / (cos(atan(r * sin(abs(belta)) / (r - r * cos(belta))) * 180 / PI));
}
